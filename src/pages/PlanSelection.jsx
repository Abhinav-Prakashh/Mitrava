    ...
    <div className="min-h-screen bg-surface flex flex-col">
      <header className="sticky top-0 z-50 bg-surface/80 backdrop-blur-xl shadow-soft">
        <div className="flex items-center justify-between px-6 lg:px-10 py-4 max-w-7xl mx-auto">
          ...
          <button onClick={() => navigate('/onboarding')} className="flex items-center gap-2 text-sm font-semibold text-on-surface-variant hover:text-on-surface transition-colors">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 lg:px-10 py-12">
          ...
          {plans.map(plan => (
            <div key={plan.id} onClick={() => setSelected(plan.id)}
              className={`relative rounded-xl p-8 cursor-pointer transition-all
                ${selected === plan.id
                  ? 'bg-surface-container-lowest shadow-[0_0_0_2px_#0056D2] shadow-soft'
            ...
            >
              ...
              <p className="text-sm text-on-surface-variant mb-6">Max payout: <strong className="text-on-surface">₹{plan.maxDailyPayout}/day</strong></p>
        ...
        {/* Stats row */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-10 border-t border-surface-container">
          {[
            { val: '99.9%', label: 'Claims Approval' },
            { val: '140+',  label: 'Cities Covered' },
            { val: '~4min', label: 'Payout Target' },
        ...
        <div className="mt-6 text-center">
          <button onClick={() => navigate('/app/home')} className="bg-primary text-on-primary font-bold px-10 py-4 rounded-lg shadow-blue transition-transform active:scale-95 inline-flex items-center gap-2">
            Activate Coverage <span className="material-symbols-outlined text-base">arrow_forward</span>
          </button>
        </div>
      </main>
    </div>
