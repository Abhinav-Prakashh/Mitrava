...
const recentActivity = [
  ...
  { icon: 'warning',      label: 'Risk Level Alert: Koramangala', time: 'Yesterday, 11:45 PM', col: 'text-[#92600a]' },
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        <h1 className="text-4xl lg:text-5xl font-extrabold tracking-tighter text-on-surface">
          Good Morning, <span className="text-primary">Ravi.</span>
        </h1>
      </div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-on-primary/15 rounded-xl p-4">
                ...
                <p className="text-2xl font-extrabold tracking-tighter text-on-primary">₹{worker.coveragePerDay}</p>
              </div>
              <div className="bg-on-primary/15 rounded-xl p-4">
                <p className="text-xs font-bold uppercase tracking-wider text-on-primary/70 mb-1">Weekly Premium</p>
                <p className="text-2xl font-extrabold tracking-tighter text-on-primary">₹{worker.weeklyPremium}</p>
              </div>
            </div>
          </div>
        ...
        <Card>
          ...
          <div className="flex items-end gap-1.5 h-16">
            {[40,55,45,65,58,72,90].map((h,i) => (
              <div key={i} className={`flex-1 rounded-sm transition-all ${i === 6 ? 'bg-primary' : 'bg-surface-container-high'}`}
                style={{height:`${h}%`}} />
            ))}
          </div>
          <div className="mt-4 grid grid-cols-2 gap-4 pt-4 border-t border-surface-container">
            <div>
              ...
              <p className="text-xl font-extrabold tracking-tighter text-on-surface">₹{(worker.avgDailyEarnings * 7).toLocaleString()}</p>
